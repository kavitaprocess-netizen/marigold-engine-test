...
app.post('/api/resync-plan', async (req, res) => {
  ...
  try {
    ...
    // Build old-index -> ceremony-name map, so we can carry over the
    // couple's own side reassignments by NAME rather than index (indices
    // shift if the fresh ceremony list has different items/ordering).
    ...
    // Carry over checklist checked-state, matched by exact label text --
    // if a label changed at all, the item is treated as new and starts
    // unchecked (per the rule: reset on meaningful wording change).
    ...
    const newPlanData = Object.assign({}, pd, {
      S: newS,
