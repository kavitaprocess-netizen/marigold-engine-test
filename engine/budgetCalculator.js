...
function calculateSingleTraditionBudget(totalBudget, budgetAllocation) {
  return budgetAllocation.map((cat) => ({
    ...
    // Preserved from mergeInterfaithBudget's own line objects when present
    // (this function is also called directly for the single-tradition
    // case, where these fields never existed on the input in the first
    // place -- default to false/null there, same as before).
    shared: cat.shared || false,
    tradition_specific_to: cat.tradition_specific_to || null,
  }));
}
