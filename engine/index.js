...
const { mergeTraditions, mergeChecklists, generateCeremonyDerivedTasks } = require('./interfaithMerge');
...
async function generatePlan({ traditionSlugs, totalBudget, actualEventCount, jurisdiction }) {
  ...
  if (loaded.length === 1) {
    ...
    const ceremonySequence = (t.ceremony_sequence || []).map(c => ({ ...c, _sourceTradition: t.name, _sourceTraditionSlug: t.slug })).filter(c => c.active !== false);
    const derivedTasks = ceremonySequence.reduce((acc, cer) => acc.concat(generateCeremonyDerivedTasks(cer)), []);

    return {
      success: true,
      traditions: [t.slug],
      // mergeChecklists (not a plain concatenation) so the same near-match
      // dedup that runs for interfaith couples also runs here -- a single
      // tradition's own generic items (e.g. its own "book venue") still
      // collapse with the universal checklist's version rather than
      // appearing twice, the exact same bug this was built to fix, just
      // without a second tradition present.
      checklist: [...derivedTasks, ...mergeChecklists(t, null, UNIVERSAL_CHECKLIST)],
