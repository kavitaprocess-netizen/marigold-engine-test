...
function generateCeremonyDerivedTasks(ceremony) {
  ...
  (ceremony.vendor_categories || []).forEach(function(vc) {
    ...
    // Materials (ritual materials): only meaningful under the
    ...
    if (vc.category === 'Officiant / pandit / priest' && Array.isArray(vc.materials)) {
      vc.materials.forEach(function(item) {
        if (!item) return;
        tasks.push({
          ...common,
          label: `Arrange ${item}`,
