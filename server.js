...
<div id="screens">
  ...
  <div class="screen" id="q7">
    <div style="width:100%;max-width:900px;margin:0 auto;padding:0 4px">
      <div class="ql">Your ceremony plan</div>
      <div class="qt" id="q7-title">Choose your ceremonies</div>
      <div class="qs" id="q7-sub">Select the ceremonies you want. We've made an intelligent start based on your traditions.</div>
      <div id="cs-container" style="margin-top:24px"></div>
  ...
  <div class="screen" id="q8">
    <div style="overflow-y:auto;max-height:calc(100vh - 120px);padding-bottom:20px">
      <div class="conf-wrap">
  ...
  <!-- Q7: Ceremony selection -->
  <div class="screen" id="q7" style="overflow-y:auto;align-items:flex-start;justify-content:flex-start;padding-top:32px">
    <div style="width:100%;max-width:900px;margin:0 auto">
      <div class="ql">Step 7 of 8</div>
      <div class="qt" id="q7-title">Choose your ceremonies</div>
      <div class="qs" id="q7-sub">We've assigned ceremonies based on your traditions. Adjust any that aren't right — then review your plan.</div>
      <div id="cs-container" style="margin-top:24px"></div>
      <div class="cta-row" style="margin-top:28px;padding-bottom:40px">
        <button class="btn-back" onclick="goBack()">← back</button>
        <button class="cta" onclick="goToConfirmation()">Review my plan →</button>
      </div>
    </div>
  ...
  <!-- Q8: Confirmation -->
  <div class="screen" id="q8" style="overflow-y:auto;align-items:flex-start;justify-content:flex-start;padding-top:32px">
    <div style="width:100%;max-width:720px;margin:0 auto;padding-bottom:60px">
      <div class="ql">Your wedding plan</div>
      <div class="qt" id="q8-title">Here's what you've chosen</div>
      <div class="qs" id="q8-sub"></div>
      <div id="conf-container" style="margin-top:28px"></div>
...
function updateProgress() {
  ...
  const label = currentQ === '4b' ? '4b of 8' : currentQ + ' of ' + TOTAL_STEPS;
...
function goBack() {
  ...
  if (currentQ === 7) {
    ...
    q7.style.transition = 'opacity 0.25s, transform 0.25s';
    q7.style.opacity = '0'; q7.style.transform = 'translateX(50px)';
    setTimeout(function() {
      q7.classList.remove('active'); q7.style.transition=''; q7.style.opacity=''; q7.style.transform='';
...
// ceremonySelections: { tradSlug: { side1: [indices], side2: [indices], both: [indices] } }
let ceremonySelections = {};
let planCeremonies = []; // raw from engine
...
function showCeremonySelection(ceremonies) {
  ...
  // Determine traditions and their assignments
  const trad1 = S.traditions[0];
  const trad2 = S.traditions[1] || null;
  // Who owns which tradition
  const name1TradSlugs = [];
  const name2TradSlugs = [];
  if (trad2) {
    // Interfaith — use Q4b assignment
    Object.entries(S.traditionAssignment).forEach(function(e) {
      if (e[1] === 'name1') name1TradSlugs.push(e[0]);
      else name2TradSlugs.push(e[0]);
    });
    if (!name1TradSlugs.length) { name1TradSlugs.push(trad1); name2TradSlugs.push(trad2); }
  } else {
    // Single tradition — both assigned to same tradition
    name1TradSlugs.push(trad1);
    name2TradSlugs.push(trad1);
  }

  // Group ceremonies by tradition slug
  function getTradSlug(item) {
    return item._sourceTradition || item.tradition || item.source || 'universal';
  }

  // Build tradition blocks
  const tradBlocks = [];
  const allTradSlugs = trad2 ? [trad1, trad2] : [trad1];

  allTradSlugs.forEach(function(tradSlug) {
    const tradInfo = TRADS.find(function(t){return t.slug===tradSlug;}) || {label:tradSlug};
    const tradLabel = tradInfo.label;

    // Who is name1 in this tradition?
    const n1Role = S.role1; // bride or groom
    const n2Role = S.role2;

    // Filter ceremonies belonging to this tradition
    const tradCeremonies = ceremonies.filter(function(item, idx) {
      const src = getTradSlug(item);
      if (trad2) {
        // Interfaith — match by source tradition
        return src === tradSlug || src === 'universal';
      }
      return true; // Single tradition — all ceremonies
    });

    // Split by side field
    const side1Items = []; // Name 1's (matches their role)
    const side2Items = []; // Name 2's
    const bothItems = [];

    tradCeremonies.forEach(function(item, i) {
      const globalIdx = ceremonies.indexOf(item);
      const side = item.side || 'both';
      if (side === 'both' || side === 'universal') {
        bothItems.push({item:item, idx:globalIdx});
      } else if (side === n1Role) {
        side1Items.push({item:item, idx:globalIdx});
      } else if (side === n2Role) {
        side2Items.push({item:item, idx:globalIdx});
      } else {
        bothItems.push({item:item, idx:globalIdx});
      }
    });

    // Only add block if it has ceremonies
    if (side1Items.length || side2Items.length || bothItems.length) {
      tradBlocks.push({
        tradSlug: tradSlug,
        tradLabel: tradLabel,
        side1: side1Items,
    ...
    // Initialise selections (all selected by default)
    if (!ceremonySelections[tradSlug]) {
      ceremonySelections[tradSlug] = { side1: {}, side2: {}, both: {} };
      side1Items.forEach(function(ci){ ceremonySelections[tradSlug].side1[ci.idx] = true; });
      side2Items.forEach(function(ci){ ceremonySelections[tradSlug].side2[ci.idx] = true; });
      bothItems.forEach(function(ci){ ceremonySelections[tradSlug].both[ci.idx] = true; });
    }
  });
  ...
  container.innerHTML = tradBlocks.map(function(block) {
    const tradHeader = trad2
      ? '<div class="cs-tradition-label">' + block.tradLabel + ' ceremonies</div>'
      : '';

    function csItem(ci, group) {
      const sel = ceremonySelections[block.tradSlug][group][ci.idx] !== false;
      const name = ci.item.name || ci.item.event || '';
      const timing = ci.item.timing || '';
      return '<div class="cs-item ' + (sel ? 'selected' : 'deselected') + '" onclick="toggleCs('' + block.tradSlug + '','' + group + '',' + ci.idx + ',this)">' +
        '<div class="cs-check"><span class="cs-check-tick">&#10003;</span></div>' +
        '<div><div class="cs-item-name">' + name + '</div>' +
        (timing ? '<div class="cs-item-timing">' + timing + '</div>' : '') +
        '</div></div>';
    }

    const col1Html = block.side1.map(function(ci){return csItem(ci,'side1');}).join('');
    const col2Html = block.side2.map(function(ci){return csItem(ci,'side2');}).join('');
    const bothHtml = block.both.map(function(ci){return csItem(ci,'both');}).join('');

    return '<div class="cs-tradition-block">' +
      tradHeader +
      '<div class="cs-cols">' +
        '<div>' +
          '<div class="cs-col-header">' + n1 + ''s ceremonies</div>' +
          (col1Html || '<div style="font-size:12px;color:var(--muted);font-style:italic;padding:8px 0">None specific</div>') +
        '</div>' +
        '<div>' +
          '<div class="cs-col-header">' + n2 + ''s ceremonies</div>' +
          (col2Html || '<div style="font-size:12px;color:var(--muted);font-style:italic;padding:8px 0">None specific</div>') +
        '</div>' +
      '</div>' +
      (bothHtml ? '<div class="cs-shared-label">For both of you</div><div class="cs-shared-grid">' + bothHtml + '</div>' : '') +
    '</div>';
  }).join('');
}

function toggleCs(tradSlug, group, idx, el) {
  const current = ceremonySelections[tradSlug][group][idx] !== false;
  ceremonySelections[tradSlug][group][idx] = !current;
  el.classList.toggle('selected', !current);
  el.classList.toggle('deselected', current);
  const tick = el.querySelector('.cs-check-tick');
  if (tick) tick.style.display = !current ? 'block' : 'none';
}
...
function goToConfirmation() {
  ...
  q7.classList.add('exit-left'); q7.classList.remove('active');
  setTimeout(function() {
    q7.classList.remove('exit-left');
    q8.classList.add('active');
  }, 260);
  currentQ = 8;
  updateProgress();
}
...
function goBackFromConf() {
  const q8 = document.getElementById('q8');
  const q7 = document.getElementById('q7');
  q8.style.transition = 'opacity 0.25s, transform 0.25s';
  q8.style.opacity = '0';
  q8.style.transform = 'translateX(50px)';
  setTimeout(function() {
    q8.classList.remove('active');
    q8.style.transition = '';
    q8.style.opacity = '';
    q8.style.transform = '';
    q7.classList.add('active');
...
function getSelectedCeremonies() {
  // Return array of selected ceremony objects
  const selected = [];
  const seen = new Set();
  Object.values(ceremonySelections).forEach(function(groups) {
    ['side1','side2','both'].forEach(function(group) {
      Object.entries(groups[group]).forEach(function(e) {
        const idx = parseInt(e[0]);
        const isSelected = e[1] !== false;
        if (isSelected && !seen.has(idx)) {
          seen.add(idx);
...
function buildConfirmation() {
  const selected = getSelectedCeremonies();
  const n1 = S.name1 || 'Partner one';
  const n2 = S.name2 || 'Partner two';
  const trad1 = S.traditions[0];
  const trad2 = S.traditions[1] || null;

  document.getElementById('q8-sub').textContent =
    selected.length + ' ceremonies selected · ' +
    S.budget.toLocaleString('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}) + ' budget · ' +
    S.guests + ' guests';

  const allTradSlugs = trad2 ? [trad1, trad2] : [trad1];
  const n1Role = S.role1;
  const n2Role = S.role2;
  ...
  allTradSlugs.forEach(function(tradSlug) {
    const tradInfo = TRADS.find(function(t){return t.slug===tradSlug;}) || {label:tradSlug};
    const tradCerems = selected.filter(function(item) {
      const src = item._sourceTradition || item.tradition || item.source || '';
      return !trad2 || src === tradSlug || src === 'universal' || src === '';
    });
    if (!tradCerems.length) return;

    const side1 = tradCerems.filter(function(i){ return (i.side||'both') === n1Role; });
    const side2 = tradCerems.filter(function(i){ return (i.side||'both') === n2Role; });
    const both = tradCerems.filter(function(i){ return !['bride','groom'].includes(i.side||'both'); });

    function confItem(item) {
      return '<div class="conf-item">' + (item.name||item.event||'') +
        (item.timing ? '<span class="conf-item-timing"> &middot; ' + item.timing + '</span>' : '') +
        '</div>';
    }
    ...
    html += '<div class="conf-block">' +
      ...
      '<div class="conf-cols">' +
        '<div><div class="conf-col-header">' + n1 + ''s ceremonies</div>' +
          (side1.length ? side1.map(confItem).join('') : '<div class="conf-item" style="opacity:0.4">None selected</div>') +
        '</div>' +
        '<div><div class="conf-col-header">' + n2 + ''s ceremonies</div>' +
          (side2.length ? side2.map(confItem).join('') : '<div class="conf-item" style="opacity:0.4">None selected</div>') +
        '</div>' +
      '</div>' +
      (both.length ? '<div class="conf-shared-header">For both of you</div>' + both.map(confItem).join('') : '') +
    '</div>';
  });

  // Budget adjusted by ceremony count
  const totalCerems = planCeremonies.length || 1;
  const selectedCount = selected.length;
  const ratio = Math.max(0.5, Math.min(1, selectedCount / totalCerems));
  const adjustedBudget = Math.round(S.budget * ratio);

  html += '<div class="conf-budget">' +
    '<div class="conf-budget-total">' + adjustedBudget.toLocaleString('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}) + '</div>' +
    '<div class="conf-budget-sub">estimated budget · adjusted for ' + selectedCount + ' of ' + totalCerems + ' ceremonies</div>' +
    (S.plan && S.plan.budget ? buildConfBudgetBars(S.plan.budget, adjustedBudget, ratio) : '') +
  '</div>';

  document.getElementById('conf-container').innerHTML = html;
}

function buildConfBudgetBars(budgetData, adjustedTotal, ratio) {
  const items = Array.isArray(budgetData) ? budgetData : (budgetData.budget_lines || []);
  if (!items.length) return '';
  return items.slice(0,8).map(function(item) {
    const cat = item.category || item.name || '';
    const amt = Math.round((item.suggested_amount || (adjustedTotal * ((item.pct_low||0)+(item.pct_high||0))/2/100)) * ratio);
    const pct = adjustedTotal > 0 ? amt/adjustedTotal*100 : 0;
    return '<div class="conf-budget-row">' +
      '<div class="conf-budget-cat">' + cat + '</div>' +
      '<div class="conf-budget-track"><div class="conf-budget-fill" style="width:' + Math.min(pct*2,100) + '%"></div></div>' +
      '<div class="conf-budget-amt">' + amt.toLocaleString('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}) + '</div>' +
    '</div>';
  }).join('');
}

function buildFinalPlan() {
  // Pass selected ceremonies to the results page
  const selected = getSelectedCeremonies();
  if (S.plan) {
