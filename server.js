...
function removeItem(field,index){if(!confirm('Remove this item?'))return;workingData[field].splice(index,1);refreshLists();}
...
async function cancelEditing(){
  if(!confirm('Discard this editing copy? All unsaved changes will be lost.'))return;
  try{
...
async function approveVersion(){
  const notes=prompt('Approval notes (optional):')||'Approved via advisor review interface.';
  try{currentVersion=await api('/versions/'+currentVersion.id+'/approve','POST',{notes});toast('Version approved — now live','success');allVersions=await api('/traditions/'+currentTradition.id+'/versions');await loadTraditions();renderMain();}
  catch(e){toast('Failed: '+e.message,'error');}
}
async function rejectVersion(){
  const notes=prompt('Rejection reason (required):');if(!notes)return;
